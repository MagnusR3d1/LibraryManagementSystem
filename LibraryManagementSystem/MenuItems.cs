                ...
                if(connect.State != ConnectionState.Open)
                {
                    try
                    {
                        ...
                        string insertData = "INSERT INTO issues " +
                            "(issued_id, full_name, contact, email, book_title, author, status, issue_date, return_date, date_insert) " +
                    ...
                    if (check == DialogResult.Yes)
                    {
                        try
                        {
                            ...
                            string updateData = "UPDATE issues SET full_name = @fullName, contact = @contact, email = @email" +
                                ...
                                ", return_date = @returnDate, date_update = @dateUpdate WHERE issued_id = @issueID";
                ...
                if (connect.State != ConnectionState.Open)
                {
                    ...
                    if (check == DialogResult.Yes)
                    {
                        try
                        {
                            connect.Open();
                            DateTime today = DateTime.Today;
                            string updateData = "UPDATE issues SET date_delete = @dateDelete WHERE issued_id = @issueID";
